...
Daily summarizer for podcasts you subscribe to in the macOS Podcasts app. Fetches new episodes from each show's RSS feed, gets a transcript (or falls back to the episode description), summarizes with DeepSeek, and writes one Markdown file per episode under `summaries/YYYY-MM-DD/`.
...
Summaries land at `summaries/<published_date>/<show-slug>__<episode-slug>.md`. Each file is YAML frontmatter followed by sections:
...
summaries/              # generated by daily-digest (gitignored)
```
